...
class World:
    ...
    def __init__(self, hallWidths, hallLengths, turns,
        ...
        self.action_space = spaces.Box(
            low=-MAX_TURNING_INPUT, high=MAX_TURNING_INPUT, shape=(1,), dtype=np.float32)
    ...
    def reset(self, side_pos=None, pos_noise=0.2, heading_noise=0.1, y_noise=0):
        self.curHall = 0
    ...
    def bicycle_dynamics(self, x, t, u, delta, turn):
        ...
        dtheta_ldt = x[2] * np.cos(np.arctan(CAR_CENTER_OF_MASS *
                                             np.tan(delta) / CAR_LENGTH)) * np.tan(delta) / CAR_LENGTH
        ...
        dtheta_gdt = x[2] * np.cos(np.arctan(CAR_CENTER_OF_MASS *
                                             np.tan(delta) / CAR_LENGTH)) * np.tan(delta) / CAR_LENGTH
    ...
    def step(self, delta, throttle=CONST_THROTTLE, x_noise=0, y_noise=0, v_noise=0, theta_noise=0):
        ...
        # simulate dynamics
        x0 = [self.car_dist_s, self.car_dist_f, self.car_V, self.car_heading,
              self.car_global_x, self.car_global_y, self.car_global_heading]
        t = [0, self.time_step]
        ...
        new_x = odeint(self.bicycle_dynamics_no_beta, x0, t, args=(
            throttle, delta * np.pi / 180, self.turns[self.curHall],))
           ...
           self.car_dist_f > wall_dist:
            ...
            if self.car_dist_f > LIDAR_RANGE:
                ...
                reward += MIDDLE_REWARD_GAIN * \
                    abs(self.car_dist_s - self.hallWidths[self.curHall] / 2.0)
        ...
        dot_prod_top = normal_to_top_wall[0] * self.car_dist_f + \
            normal_to_top_wall[1] * self.car_dist_s
        ...
        if self.car_dist_s > self.hallWidths[self.curHall] and not terminal:
            ...
            reward += MIDDLE_REWARD_GAIN * \
                abs(s_new - self.hallWidths[(self.curHall+1) % self.numHalls] / 2.0)
        ...
        if self.car_dist_s > LIDAR_RANGE:
            ...
            # update corner coordinates
            (self.outer_x, self.outer_y, self.inner_x,
             self.inner_y) = self.next_corner_coordinates(flip_sides)
        ...
        if self.state_feedback:
            ...
            if self.car_dist_s <= self.hallWidths[self.curHall]:
                ...
                (next_outer_x, next_outer_y, next_inner_x,
                 next_inner_y) = self.next_corner_coordinates(flip_sides)

                corner_dist = np.sqrt((next_outer_x - next_inner_x) ** 2 +
                                      (next_outer_y - next_inner_y) ** 2)
                wall_dist = np.sqrt(corner_dist ** 2 -
                                    self.hallWidths[(self.curHall+2) % self.numHalls] ** 2)
    ...
    def next_car_states(self, flip_sides):
        ...
        if corner_angle > np.pi/2:
            inner_angle = corner_angle - \
                math.atan(np.abs(self.car_dist_f) / self.car_dist_s) - np.pi/2

        next_dist_s = np.sin(inner_angle) * dist_to_outer
        if flip_sides:
            next_dist_s = self.hallWidths[(self.curHall+1) % self.numHalls] - next_dist_s

        next_dist_f = self.hallLengths[(self.curHall+1) % self.numHalls] - \
    ...
    def next_corner_coordinates(self, flip_sides):
        ...
        if flip_sides:
            starting_corner_dist = np.sqrt(
                (self.outer_x - self.inner_x) ** 2 + (self.outer_y - self.inner_y) ** 2)
            wall_dist = np.sqrt(starting_corner_dist ** 2 -
                                self.hallWidths[(self.curHall+1) % self.numHalls] ** 2)

            next_outer_x = self.inner_x + \
                np.cos(self.cur_hall_heading) * \
                (self.hallLengths[(self.curHall+1) % self.numHalls] - wall_dist)
            next_outer_y = self.inner_y + \
                np.sin(self.cur_hall_heading) * \
                (self.hallLengths[(self.curHall+1) % self.numHalls] - wall_dist)
        else:
            next_outer_x = self.outer_x + \
                np.cos(self.cur_hall_heading) * self.hallLengths[(self.curHall+1) % self.numHalls]
            next_outer_y = self.outer_y + \
                np.sin(self.cur_hall_heading) * self.hallLengths[(self.curHall+1) % self.numHalls]
        ...
        out_wall_proj_length = np.abs(
            self.hallWidths[self.curHall] / np.sin(self.turns[(self.curHall+1) % self.numHalls]))
        ...
        in_wall_proj_length = np.abs(self.hallWidths[(
            self.curHall+1) % self.numHalls] / np.sin(self.turns[(self.curHall+1) % self.numHalls]))
    ...
    def scan_lidar(self):
        ...
        if car_dist_f_inner >= 0:
            ...
            if self.car_dist_f >= 0:
                ...
                theta_outer = 90 + np.arctan(np.abs(self.car_dist_f) /
                                             float(self.car_dist_s)) * 180 / np.pi
                ...
                if self.turns[self.curHall] < 0:
                    if angle <= theta_r:
                        ...
                        dist_to_bottom_wall = self.hallWidths[(
                            self.curHall+1) % self.numHalls] - dist_to_outer * np.sin(outer_angle)
    ...
    def plot_lidar(self, show_halls=True, zero_dist_rays=False, savefilename=''):
        ...
        for curAngle in theta_t:
            ...
            lidX.append(self.car_global_x + data[index] *
                        np.cos(curAngle * np.pi / 180 + self.car_global_heading))
            lidY.append(self.car_global_y + data[index] *
                        np.sin(curAngle * np.pi / 180 + self.car_global_heading))
