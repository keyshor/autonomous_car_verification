...
def generate_data():
    with open('straight_little_notgoal.csv', mode='x', newline='') as straight_little_notgoal_file, \
            open('straight_little_notgoal_position.csv', mode='x', newline='') as straight_little_notgoal_position_file, \
            open('straight_little_goal.csv', mode='x', newline='') as straight_little_goal_file, \
            open('straight_little_goal_position.csv', mode='x', newline='') as straight_little_goal_position_file, \
            open('right_little_notgoal.csv', mode='x', newline='') as right_little_notgoal_file, \
            open('right_little_notgoal_position.csv', mode='x', newline='') as right_little_notgoal_position_file, \
            open('right_little_goal.csv', mode='x', newline='') as right_little_goal_file, \
            open('right_little_goal_position.csv', mode='x', newline='') as right_little_goal_position_file, \
            open('left_little_notgoal.csv', mode='x', newline='') as left_little_notgoal_file, \
            open('left_little_notgoal_position.csv', mode='x', newline='') as left_little_notgoal_position_file, \
            open('left_little_goal.csv', mode='x', newline='') as left_little_goal_file, \
            open('left_little_goal_position.csv', mode='x', newline='') as left_little_goal_position_file, \
        ...
        straight_little_notgoal_position = csv.writer(straight_little_notgoal_position_file)
        straight_little_goal = csv.writer(straight_little_goal_file)
        straight_little_goal_position = csv.writer(straight_little_goal_position_file)
        right_little_notgoal = csv.writer(right_little_notgoal_file)
        right_little_notgoal_position = csv.writer(right_little_notgoal_position_file)
        right_little_goal = csv.writer(right_little_goal_file)
        right_little_goal_position = csv.writer(right_little_goal_position_file)
        left_little_notgoal = csv.writer(left_little_notgoal_file)
        left_little_notgoal_position = csv.writer(left_little_notgoal_position_file)
        left_little_goal = csv.writer(left_little_goal_file)
        left_little_goal_position = csv.writer(left_little_goal_position_file)
        ...
        while cur_dist_f >= 5 + width - 1.5:
            for _ in range(iter_batch):
                ...
                position = [w.car_global_x, w.car_global_y, w.car_global_heading]
                obs = normalize(w.scan_lidar())
                straight_little_notgoal.writerow(obs)
                straight_little_notgoal_position.writerow(position)
        ...
        while cur_dist_f >= 5 + width - 2:
            for _ in range(iter_batch):
                ...
                if abs(w.car_dist_s - cur_dist_s) < 0.01 and abs(w.car_heading - cur_heading) < 0.02:
                    ...
                    straight_little_goal_position.writerow(position)
                else:
                    straight_little_notgoal.writerow(obs)
                    straight_little_notgoal_position.writerow(position)
                right_little_notgoal.writerow(obs)
                right_little_notgoal_position.writerow(position)
                big_right.writerow(obs)
            for _ in range(50 * iter_batch):
                w.set_state_local(
                        cur_dist_s + np.random.uniform(-3 * goal_pos_range, 3 * goal_pos_range),
                        cur_dist_f,
                        cur_heading + np.random.uniform(-3 * goal_heading_range, 3 * goal_heading_range)
                        )
                position = [w.car_global_x, w.car_global_y, w.car_global_heading]
                obs = normalize(w.scan_lidar())
                if abs(w.car_dist_s - cur_dist_s) < 0.01 and abs(w.car_heading - cur_heading) < 0.02:
        ...
        while cur_dist_s <= width + 0.5:
            for _ in range(iter_batch):
                ...
                if abs(w.car_dist_f - cur_dist_f) < 0.01 and abs(w.car_heading - cur_heading) < 0.02:
                    ...
                    right_little_goal_position.writerow(position)
                else:
                    right_little_notgoal.writerow(obs)
                    right_little_notgoal_position.writerow(position)
                straight_little_notgoal.writerow(obs)
            ...
            for _ in range(50 * iter_batch):
                w.set_state_local(
                        cur_dist_s,
                        cur_dist_f + np.random.uniform(-3 * goal_pos_range, 3 * goal_pos_range),
                        cur_heading + np.random.uniform(-3 * goal_heading_range, 3 * goal_heading_range)
                        )
                position = [w.car_global_x, w.car_global_y, w.car_global_heading]
                obs = normalize(w.scan_lidar())
        ...
        while cur_dist_f >= 5 + width - 2:
            for _ in range(iter_batch):
                ...
                left_little_notgoal_position.writerow(position)
        ...
        while cur_dist_s <= width + 0.5:
            for _ in range(iter_batch):
                ...
                if abs(w.car_dist_f - cur_dist_f) < 0.01 and abs(w.car_heading - cur_heading) < 0.02:
                    ...
                    left_little_goal_position.writerow(position)
                else:
                    left_little_notgoal.writerow(obs)
                    left_little_notgoal_position.writerow(position)
                straight_little_notgoal.writerow(obs)
