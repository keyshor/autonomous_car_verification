...
EXIT_POS_LB = 0.835
EXIT_POS_UB = 0.867
EXIT_HEADING_LB = -0.075
EXIT_HEADING_UB = -0.035
...
def writeDnnJumps(stream, numInputs):
    ...
    stream.write('\t\tguard { clock = 0 _f5 - _f1 <= 0 _f4 - _f1 <= 0 _f3 - _f1 <= 0 _f2 - _f1 <= 0 }\n')
    stream.write('\t\treset { ')
    for i in range(numInputs):
        stream.write('_f{ray}\' := f_temp{ray} '.format(ray=i+1))
    ...
    stream.write('\t\tDNN1 -> DNN7\n')
    stream.write('\t\tguard { clock = 0 _f5 - _f2 <= 0 _f4 - _f2 <= 0 _f3 - _f2 <= 0 _f1 - _f2 <= 0 }\n')
    stream.write('\t\treset { ')
    for i in range(numInputs):
        stream.write('_f{ray}\' := f_temp{ray} '.format(ray=i+1))
    ...
    stream.write('\t\tDNN1 -> DNN7\n')
    stream.write('\t\tguard { clock = 0 _f5 - _f3 <= 0 _f4 - _f3 <= 0 _f2 - _f3 <= 0 _f1 - _f3 <= 0 }\n')
    stream.write('\t\treset { ')
    for i in range(numInputs):
        stream.write('_f{}\' := f_temp{} '.format(i+1, numInputs-i))
    ...
    stream.write('\t\tguard { clock = 0 _f5 - _f4 <= 0 _f3 - _f4 <= 0 _f2 - _f4 <= 0 _f1 - _f4 <= 0 }\n')
    stream.write('\t\treset { ')
    for i in range(numInputs):
        stream.write('_f{ray}\' := f_temp{ray} '.format(ray=i+1))
    ...
    stream.write('\t\tDNN1 -> DNN8\n')
    stream.write('\t\tguard { clock = 0 _f4 - _f5 <= 0 _f3 - _f5 <= 0 _f2 - _f5 <= 0 _f1 - _f5 <= 0 }\n')
    stream.write('\t\treset { ')
    for i in range(numInputs):
        stream.write('_f{}\' := f_temp{} '.format(i+1, numInputs-i))
...
def writeEndJump(stream):
    ...
    stream.write('\t\tguard { clock = ' + str(TIME_STEP) + ' ax = 0 y2 <= ' +
                 str(EXIT_DISTANCE) + ' y1 <= ' + str(EXIT_POS_LB) + '}\n')
    stream.write('\t\treset { ')
    stream.write('clock\' := 0')
    stream.write('}\n')
    stream.write('\t\tinterval aggregation\n')
    ...
    stream.write('\t\tguard { clock = ' + str(TIME_STEP) + ' ax = 0 y2 <= ' +
                 str(EXIT_DISTANCE) + ' y1 >= ' + str(EXIT_POS_UB) + '}\n')
    stream.write('\t\treset { ')
    stream.write('clock\' := 0')
    stream.write('}\n')
    stream.write('\t\tinterval aggregation\n')
    ...
    stream.write('\t\tguard { clock = ' + str(TIME_STEP) + ' ax = 0 y2 <= ' + str(EXIT_DISTANCE) +
    ...
    stream.write('\t\t_cont_m2 ->  m_end_yh\n')
    stream.write('\t\tguard { clock = ' + str(TIME_STEP) + ' ax = 0' +
                 ' y2 <= ' + str(EXIT_DISTANCE - 0.25) + ' }\n')
    stream.write('\t\treset { ')
    stream.write('clock\' := 0')
    stream.write('}\n')
    stream.write('\t\tinterval aggregation\n')
...
def main(argv):
    ...
    plantPickle = '../plant_models/dynamics_nn_entry_' + name + '{}.pickle'.format(numRays)
    ...
    big = '../simulator/decomposed_mode_predictor/big.yml'
    straight_little = '../simulator/decomposed_mode_predictor/straight_little.yml'
    square_right_little = '../simulator/decomposed_mode_predictor/square_right_little.yml'
    square_left_little = '../simulator/decomposed_mode_predictor/square_left_little.yml'
    sharp_right_little = '../simulator/decomposed_mode_predictor/sharp_right_little.yml'
    sharp_left_little = '../simulator/decomposed_mode_predictor/sharp_left_little.yml'
    square_controller = '../verisig/tanh64x64_right_turn_lidar.yml'
    sharp_controller = '../verisig/tanh64x64_sharp_turn_lidar.yml'
    ...
    while cur_process < len(all_args):
        ...
        # first poll
        for process_id in cur_runnning:
