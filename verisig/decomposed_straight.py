...
POS_LB_STABLE = 0.824
POS_UB_STABLE = 0.827
HEADING_LB_STABLE = -0.0016
HEADING_UB_STABLE = 0.00041
...
def writeEndJump(stream):
    ...
    stream.write('\t\t_cont_m2 ->  m_end_segment\n')
    stream.write('\t\tguard { clock = ' + str(TIME_STEP) + ' k = ' +
                 str(NUM_STEPS-1) + ' segment_mode >= ' + str(1) + ' }\n')
    stream.write('\t\treset { ')
    stream.write('clock\' := 0')
    stream.write('}\n')
    stream.write('\t\tinterval aggregation\n')
...
def writeComposedSystem(filename, initProps, dnn,
    ...
    with open(filename, 'w') as stream:
        ...
        writeEndMode(stream, plant_states, 'm_end_segment')
