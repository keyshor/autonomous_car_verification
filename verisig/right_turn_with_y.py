...
def writeDnnJumps(stream, numInputs):
    ...
    # jump from m0 to mode predictor-------------------------------------------
    ...
    # jump from controller to m1-----------------------------------
    stream.write('\t\tDNN2 -> m1\n')
    stream.write('\t\tguard { clock = 0 }\n')
    stream.write('\t\treset { ')
...
def writeEndJump(stream):
    ...
    stream.write('\t\tguard { clock = ' + str(TIME_STEP) + ' ax = 1 y2 <= ' +
                 str(EXIT_DISTANCE) + ' y1 <= ' + str(POS_LB) + '}\n')
    stream.write('\t\treset { ')
    stream.write('clock\' := 0')
    stream.write('}\n')
    stream.write('\t\tinterval aggregation\n')
    ...
    stream.write('\t\tguard { clock = ' + str(TIME_STEP) + ' ax = 1 y2 <= ' +
                 str(EXIT_DISTANCE) + ' y1 >= ' + str(POS_UB) + '}\n')
    stream.write('\t\treset { ')
    stream.write('clock\' := 0')
    stream.write('}\n')
    stream.write('\t\tinterval aggregation\n')
    ...
    stream.write('\t\tguard { clock = ' + str(TIME_STEP) + ' ax = 1 y2 <= ' + str(EXIT_DISTANCE) +
                 ' y4 <= ' + str(HEADING_LB) + '}\n')
    stream.write('\t\treset { ')
    stream.write('clock\' := 0')
    stream.write('}\n')
    ...
    stream.write('\t\tguard { clock = ' + str(TIME_STEP) + ' ax = 1' +
...
def writeComposedSystem(filename, initProps, dnn, mode_dnn,
...
def main(argv):
    ...
    with open(modeYaml, 'rb') as f:

        mode_dnn = yaml.load(f)
